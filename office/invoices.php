...
if (isset($_POST['gen_invoice'])) {
    ...
    if ($conn->query($sql)) {
        $status_msg = "<div class='alert-success'><i class='fas fa-check-double'></i> Success: Invoice $inv_no created!</div>";
    } else {
        // This will show you if there is a column name mismatch
        $status_msg = "<div style='background:#fee2e2; color:#991b1b; padding:15px; border-radius:12px;'>
                        <strong>Error:</strong> " . $conn->error . "
                      </div>";
    }
}
// 3. HANDLE MARK AS PAID (With Error Reporting)
if (isset($_GET['pay_inv'])) {
    ...
    if ($inv) {
        ...
        if ($conn->query($sql_rcp)) {
            // ONLY if receipt is saved, update the invoice
            ...
            $status_msg = "<div style='background:#f8d7da; color:#721c24; padding:20px; border-radius:12px; border:1px solid #f5c6cb;'>
                            <strong>❌ Database Error:</strong> " . $conn->error . " <br>
                            <em>Check if your 'receipts' table columns match the names in the code.</em>
                          </div>";
        }
    }
}
                        ...
                        <tbody>
                            ...
                            if ($invoices->num_rows > 0) {
                                while ($inv = $invoices->fetch_assoc()) {
                                    ...
                                    // Logic: Show the button ONLY if the invoice is Unpaid
                                    $action_link = "";
                                    if ($inv['status'] == 'Unpaid') {
                                        $action_link = "<a href='invoices.php?pay_inv={$inv['invoice_no']}' 
                                   ...
                                   onclick='return confirm(\"Are you sure you want to mark {$inv['invoice_no']} as Paid and generate a receipt?\")'>
                                   <i class='fas fa-check'></i> Mark Paid
                                </a>";
                                    } else {
                                        $action_link = "<span style='margin-left:10px; font-size:11px; color:#64748b;'><i class='fas fa-file-check'></i> Receipt Created</span>";
                                    }
