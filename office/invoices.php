...
if (isset($_POST['gen_invoice'])) {
    ...
    $base_amount = mysqli_real_escape_string($conn, $_POST['amount']);
    $tax_rate = mysqli_real_escape_string($conn, $_POST['tax_rate']); // Now from manual input
    $due_date = mysqli_real_escape_string($conn, $_POST['due_date']);
    $today = date('Y-m-d H:i:s');

    // CALCULATION
    $total_amount = $base_amount + ($base_amount * ($tax_rate / 100));
    ...
    $sql = "INSERT INTO invoices (invoice_no, client_id, service_name, amount, tax_rate, due_date, status, created_at) 
            VALUES ('$inv_no', '$client_id', '$service', '$total_amount', '$tax_rate', '$due_date', 'Unpaid', '$today')";

    if ($conn->query($sql)) {
        $status_msg = "<div class='alert-success'><i class='fas fa-check-double'></i> Success: Invoice $inv_no created for ₹" . number_format($total_amount, 2) . "!</div>";
    } else {
        $status_msg = "<div style='background:#fee2e2; color:#991b1b; padding:15px; border-radius:12px;'><strong>Error:</strong> " . $conn->error . "</div>";
    }
}
...
if (isset($_GET['pay_inv'])) {
    ...
    if ($inv) {
        $total_to_pay = $inv['amount']; 
        $cid = $inv['client_id'];
        $rcp_res = $conn->query("SELECT id FROM receipts");
        $rcp_count = ($rcp_res) ? $rcp_res->num_rows + 1 : 1;
        ...
        $sql_rcp = "INSERT INTO receipts (receipt_no, invoice_no, client_id, amount_paid, payment_mode) 
                    VALUES ('$rcp_no', '$inv_no', '$cid', '$total_to_pay', 'Cash/Online')";

        if ($conn->query($sql_rcp)) {
            $conn->query("UPDATE invoices SET status = 'Paid' WHERE invoice_no = '$inv_no'");
            $status_msg = "<div class='alert-success'>✅ Invoice $inv_no Paid! Receipt $rcp_no created.</div>";
        }
    }
}
?>
        ...
        <div class="grid">
            <div class="card">
                ...
                <form method="POST">
                    ...
                    <input type="text" name="service" value="<?php echo htmlspecialchars($auto_svc); ?>" placeholder="e.g. Audit Fees" required>

                    <div style="display: flex; gap: 10px;">
                        <div style="flex: 2;">
                            <label style="font-size:11px; font-weight:700;">BASE AMOUNT (₹)</label>
                            <input type="number" step="0.01" name="amount" value="<?php echo $auto_amt; ?>" placeholder="0.00" required>
                        </div>
                        <div style="flex: 1;">
                            <label style="font-size:11px; font-weight:700;">TAX (%)</label>
                        ...
                        while ($inv = $invoices->fetch_assoc()) {
                            ...
                            echo "<tr>
                                    ...
                                    <td style='font-weight:700;'>
                                        ...
                                        <div style='font-size:10px; font-weight:normal; color:#64748b;'>Incl. {$inv['tax_rate']}% Tax</div>
                                    </td>
                                    <td>
                                        <span class='$status_badge'>{$inv['status']}</span>
                                        " . ($inv['status'] == 'Unpaid' ? "
                                        <a href='invoices.php?pay_inv={$inv['invoice_no']}' 
                                           style='margin-left:10px; color:var(--navy); font-size:11px; text-decoration:none; border:1px solid #e2e8f0; padding:4px 8px; border-radius:5px;' 
