...
// 1. Fetch Master Invoice & Client Info
...
$invoice_db_id = $data['id'];

// 2. Fetch All Items for this Invoice
$items_res = $conn->query("SELECT * FROM invoice_items WHERE invoice_id = '$invoice_db_id'");
$items = [];
...
// --- CALCULATION LOGIC ---
$calculated_taxable_value = 0;
$calculated_total_tax = 0;

foreach ($items as $item) {
    $calculated_taxable_value += (float)$item['amount'];
    $calculated_total_tax += (float)$item['tax_value'];
}
...
$total     = $calculated_taxable_value + $calculated_total_tax;
$paid      = (float)($data['paid_amount'] ?? 0);
$balance   = $total - $paid;

$display_tax_rate = ($calculated_taxable_value > 0) ? ($calculated_total_tax / $calculated_taxable_value) * 100 : 0;
    ...
    <div class="page">
        <div class="header">
            <div class="header-left">
                ...
                <strong>Invoice Date:</strong> <?php echo date('d-m-Y', strtotime($data['invoice_date'])); ?><br>
                <strong>Due Date:</strong> <span style="color: #555;"><?php echo date('d-m-Y', strtotime($data['due_date'])); ?></span><br>
        ...
        <table class="main-table">
            ...
            <tbody>
                ...
                $count = 1;
                foreach ($items as $item):
                    $item_basic = (float)$item['amount'];
                    $item_tax = (float)$item['tax_value'];
                    $item_total = $item_basic + $item_tax;
                ?>
                    <tr>
                        <td align="center"><?php echo $count++; ?></td>
                        <td>
                            <strong><?php echo strtoupper($item['service_name']); ?></strong>
                        ...
                        <td>9982</td>
                        <td>₹<?php echo number_format($item_basic, 2); ?></td>
                        <td><?php echo (float)$item['tax_rate']; ?>%</td>
                        <td align="right"><strong>₹<?php echo number_format($item_total, 2); ?></strong></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        ...
        <div class="summary-container">
            ...
            <table class="summary-table">
                <tr>
                    <td>Taxable Value:</td>
                    <td align="right">₹<?php echo number_format($calculated_taxable_value, 2); ?></td>
                </tr>
                <?php if ($tax_type == 'IGST'): ?>
                    <tr>
                        <td>IGST (<?php echo round($display_tax_rate, 2); ?>%):</td>
                        <td align="right">₹<?php echo number_format($igst_amt, 2); ?></td>
                    </tr>
                <?php else: ?>
                    <tr>
                        <td>CGST (<?php echo round($display_tax_rate / 2, 2); ?>%):</td>
                        <td align="right">₹<?php echo number_format($cgst_amt, 2); ?></td>
                    </tr>
                    <tr>
                        <td>SGST (<?php echo round($display_tax_rate / 2, 2); ?>%):</td>
